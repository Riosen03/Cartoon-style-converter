...
* 윤곽선이 뚜렷하고 객체 간 색 대비가 비교적 큰 이미지
* 사진처럼 픽셀 수가 애초에 굉장히 큰 이미지

### 잘 안 되는 경우

* 색 대비가 낮거나 배경과 객체의 구분이 어려운 이미지(edge 탐지가 잘 안됨)
* 객체가 집중적으로 많거나 크기가 작은 이미지(전반적으로 색이 뭉개져서 원본 유추가 불가)

---

## 한계점

* 색 대비가 너무 낮은 영역에서는 edge를 잘 찾지 못함
* 최대한 edge 검출하기 위해 감도를 올려둬서 물체에 생긴 빛의 반사까지 탐지하는 경향이 있음
* 필터와 k-means에 의해 색을 뭉개서 물체가 원본을 추측하지 못할 정도로 부자연스럽게 표현될 수 있음
